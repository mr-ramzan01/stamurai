        ...
        <div className=''>
            {
              ...
              {store.tasks.map((task) => (
                ...
                <button className={task.status ? 'text-white rounded py-[5px] px-[20px] bg-[green] mt-[15px] block m-auto' : 'text-white rounded py-[5px] px-[20px] bg-[#0066ff] mt-[15px] block m-auto'} onClick={() => handleToggleTodo(task)}>{task.status ? "completed":'Not completed'}</button>
                <br />
                <button className='rounded py-[5px] px-[20px] bg-[red] text-white mb-[10px] block m-auto' onClick={() => handleRemoveTodo(task)}>Delete</button>
                {/* <span className="material-symbols-outlined absolute bottom-0 right-0 text-3xl cursor-pointer">edit</span> */}
            </div>
            ))}
              </div> : <h2 className='text-center text-2xl my-[30px]'>
